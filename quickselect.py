...
def partition(all_input_nums, left, right):
    ...
    temp = all_input_nums[right]
    all_input_nums[right] = all_input_nums[random_index]
    all_input_nums[random_index] = temp
...
def WriteStatus(status_message, color):
    ...
    status_writer.penup()
    status_writer.goto((BAR_WIDTH * len(all_input_nums) + SCREEN_BORDER) / 2, (max(all_input_nums) + SCREEN_BORDER) / 1.1)
...
def WriteDescription(description_message, color):
    ...
    description_writer.goto((BAR_WIDTH * len(all_input_nums) + SCREEN_BORDER) / 2, (max(all_input_nums) + SCREEN_BORDER) / 1.2)
