...
def Partition(all_input_nums, left_idx, right_idx):
    ...
    all_input_nums[less_than_idx + 1], all_input_nums[right_idx] = all_input_nums[right_idx], all_input_nums[less_than_idx + 1]

    WriteDescription("Sorting the sublist using the pivot element...", DESCRIPTION_COLOR)

    # Show the two sides we sorted based on the pivot
    for idx in range(left_idx, less_than_idx + 1):
        turtles[idx].clear()
        DrawBar(turtles[idx], BAR_WIDTH * idx, 0, all_input_nums[idx], LEFT_PIVOT_SUBLIST_COLOR)
        screen.update()
        time.sleep(bar_coloring_speed)
    turtles[less_than_idx + 1].clear()
    DrawBar(turtles[less_than_idx + 1], BAR_WIDTH * (less_than_idx + 1), 0, all_input_nums[less_than_idx + 1], PIVOT_COLOR)
    screen.update()
    for idx in range(less_than_idx + 2, right_idx + 1):
        turtles[idx].clear()
        DrawBar(turtles[idx], BAR_WIDTH * idx, 0, all_input_nums[idx], RIGHT_PIVOT_SUBLIST_COLOR)
        screen.update()
        time.sleep(bar_coloring_speed)
...
def RandomPartition(all_input_nums, left_idx, right_idx):
    ...
    WriteDescription("Selecting the random pivot element...", DESCRIPTION_COLOR)

    # Show the pivot element that we selected
    turtles[pivot_idx].clear()
    DrawBar(turtles[pivot_idx], BAR_WIDTH * pivot_idx, 0, all_input_nums[pivot_idx], PIVOT_COLOR)
    screen.update()
    time.sleep(pause_length)
...
def QuickSort(all_input_nums, left_idx, right_idx, k_val):
    ...
    WriteDescription("Selecting sublist to be sorted using the pivot element...", DESCRIPTION_COLOR)
